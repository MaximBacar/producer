...
const SpeakerContextBridge = ({
  ...
  return useMemo(
    () => <SpeakerControls className={className} playerRef={playerRefStatic} tracks={tracks} />,
...
export function Speaker({
  ...
  const tracks: Track[] = src
    ? [{ id: "beat", name: title ?? "Beat", url: src }]
    : exampleTracks.map((t) => ({ id: t.id, name: t.name, url: t.url }))
...
function SpeakerControls({
  ...
  useEffect(() => {
    const track = {
      ...
      data: { name: currentTrack.name },
    }
    playerApiRef.current.setActiveItem(track)
    precomputeWaveform(track.src)
  }, [precomputeWaveform, currentTrack.id, currentTrack.url, currentTrack.name])
